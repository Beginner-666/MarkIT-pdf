...
**[🌐 在线体验](https://你的用户名.github.io/markit-pdf/)**
...
```bash
# 或者克隆整个仓库
git clone https://github.com/你的用户名/markit-pdf.git
cd markit-pdf
# 用浏览器打开 index.html
```
...
- 点击中央上传区域，或将 PDF 文件**拖拽**到页面中

### 添加批注

1. 点击顶部工具栏的 **「批注」** 按钮，进入批注模式（按钮变为金色呼吸灯状态）
...
- **单击**已有批注：打开编辑器修改内容
- 编辑器底部点击 **「删除」** 可移除该批注
- 选中批注后可**拖拽**移动位置，拖动顶部绿点可**旋转**，拖动四角可**缩放**
...
**Q: 换了浏览器或清除缓存后，批注还在吗？**
A: 不在。批注保存在浏览器的 IndexedDB 中，清除浏览器数据后会丢失。建议定期使用「导出 PDF」功能备份。
